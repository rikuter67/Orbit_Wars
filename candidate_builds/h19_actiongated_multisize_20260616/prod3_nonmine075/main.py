...
class ProducerLiteConfig:
    ...
    size_multipliers: tuple[float, ...] = (0.75, 1.0)
...
def _tier_candidates(
    *,
    ...
    partial_target_mask: Tensor | None,
    drain: Tensor,
    floor: Tensor,
    eta_cap: Tensor,
    size_mult: float,
    ...
    if float(size_mult) < 0.999 and partial_target_mask is not None:
        valid = valid & partial_target_mask.view(1, T)
    L = 1
    C = S * T
...
def plan_lite_waves(
    *,
    ...
    tier_parts = [
        _tier_candidates(
            movement=movement, source_idx=source_idx, source_exists=source_exists,
            target_idx=target_idx, target_exists=target_exists, target_is_mine=target_is_mine,
            partial_target_mask=(~target_is_mine),
            drain=drain, floor=floor, eta_cap=eta_cap, size_mult=float(mult),
            S=S, T=T, pid=pid, garrison_status=garrison_status, prod=prod,
...
CONFIG_2P_PRODUCER_PREDICT = ProducerLiteConfig(
    horizon=18,
    max_sources_per_lane=12,
    max_offensive_targets=12,
    max_defensive_targets=4,
...
PRODUCER_MATCH_THRESHOLD = 3
ACTION_GATE_MAX_STEP = 10_000
...
class ProducerLiteMemory:
    def __init__(self) -> None:
        ...
        self.use_nonmine075: bool = False
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        if int(mem.cached_player_count) < 4 and enemy_id is not None:
            ...
            if (
                mem.producer_like_checks >= PRODUCER_MATCH_THRESHOLD
                and mem.producer_like_matches >= PRODUCER_MATCH_THRESHOLD
                and step <= ACTION_GATE_MAX_STEP
            ):
                mem.use_nonmine075 = True
        config = _config_for(mem.cached_player_count)
        if int(mem.cached_player_count) >= 4:
            config = dataclasses.replace(config, size_multipliers=(1.0,))
        elif not mem.use_nonmine075:
            config = dataclasses.replace(config, size_multipliers=(1.0,))
        row = run_turn(
        ...
        if int(mem.cached_player_count) < 4 and enemy_id is not None:
            ...
            predicted = run_turn(
                enemy_obs, config=CONFIG_2P_PRODUCER_PREDICT,
