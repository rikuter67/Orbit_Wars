...
CONFIG_2P_EARLY = dataclasses.replace(ProducerLiteConfig(), min_ships_to_launch=8.0)
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        step = int(obs_tensors["step"].item()) if "step" in obs_tensors else 0
        config = CONFIG_2P_EARLY if int(mem.cached_player_count) < 4 and step < 35 else _config_for(mem.cached_player_count)
        row = run_turn(
