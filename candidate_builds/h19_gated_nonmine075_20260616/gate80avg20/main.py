...
def _initial_feature_gate(obs_tensors: dict, player_id: int) -> bool:
    ...
    best_fast_value = max((value for value, _, margin, my_eta in rows if margin >= 1.25 and my_eta <= 9.0), default=0.0)
    avg_abs_margin_top5 = sum(margin for _, margin, _, _ in top) / max(1, len(top))
    return best_fast_value <= 80.0 and avg_abs_margin_top5 >= 20.0
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        if mem.cached_player_count is None:
            ...
            mem.use_nonmine075 = int(mem.cached_player_count) < 4 and _initial_feature_gate(
                obs_tensors, int(obs_tensors["player"].item())
            )
        config = _config_for(mem.cached_player_count)
        if int(mem.cached_player_count) < 4 and not mem.use_nonmine075:
