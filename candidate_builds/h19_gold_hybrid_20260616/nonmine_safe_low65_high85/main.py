...
def _initial_map_features(obs_tensors: dict, player_id: int) -> tuple[float, float, float]:
    ...
    def speed(ships: float) -> float:
        ships = max(float(ships), 1.0)
        return 1.0 + 5.0 * min((math.log(ships) / math.log(1000.0)), 1.0) ** 1.5

    def eta(src, tgt) -> float:
        dist = math.hypot(float(src[2]) - float(tgt[2]), float(src[3]) - float(tgt[3]))
        return max(0.0, dist - float(src[4]) - float(tgt[4])) / speed(float(src[5]))

    rows = []
    for n in neutral:
        my_eta = min(eta(m, n) for m in mine)
        en_eta = min(eta(e, n) for e in enemy)
        value = float(n[6]) * 19.0 + float(n[4]) * 2.0 - float(n[5]) * 0.25
        rows.append((value, abs(en_eta - my_eta), en_eta - my_eta, my_eta))
    top = sorted(rows, reverse=True)[:5]
    my_fast_values = [
        value for value, _, margin, my_eta in rows
        if margin >= 1.25 and my_eta <= 9.0
    ]
    best_fast_value = max(my_fast_values, default=0.0)
    avg_abs_margin_top5 = sum(margin for _, margin, _, _ in top) / max(1, len(top))
    return best_fast_value, avg_abs_margin_top5, float(len(my_fast_values))


def _use_nonmine075_2p_gate(obs_tensors: dict, player_id: int) -> bool:
    best_fast_value, avg_abs_margin_top5, my_fast_count = _initial_map_features(obs_tensors, player_id)
    high_fast_clear = best_fast_value >= 85.0 and avg_abs_margin_top5 >= 15.0 and my_fast_count >= 4.0
    low_fast_contested = best_fast_value <= 65.0 and 10.0 <= avg_abs_margin_top5 <= 20.0
    return high_fast_clear or low_fast_contested


def _use_producer_2p_gate(obs_tensors: dict, player_id: int) -> bool:
    best_fast_value, _avg_abs_margin_top5, _my_fast_count = _initial_map_features(obs_tensors, player_id)
    return best_fast_value >= 85.0
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        if mem.cached_player_count is None:
            ...
            if int(mem.cached_player_count) < 4:
                ...
                mem.use_nonmine075 = _use_nonmine075_2p_gate(obs_tensors, player_id)
                mem.use_producer_2p = (not mem.use_nonmine075) and _use_producer_2p_gate(
                    obs_tensors, player_id
                )
        config = _config_for(mem.cached_player_count)
        if int(mem.cached_player_count) < 4 and mem.use_producer_2p:
            config = CONFIG_2P_PRODUCER
        elif int(mem.cached_player_count) < 4 and not mem.use_nonmine075:
