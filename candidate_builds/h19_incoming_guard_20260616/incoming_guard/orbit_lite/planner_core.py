...
def incoming_fleet_targets(obs) -> tuple[Tensor, Tensor]:
    """Owned planets likely on the path of an observed enemy fleet.

    This is deliberately geometric and local: enemy fleets already in flight,
    projected forward from their current location along their current angle.
    """
    P = obs.P
    device = obs.device
    ...
    fleet_enemy = obs.f_alive & (obs.f_owner >= 0) & (obs.f_owner != int(obs.player_id)) & (obs.f_ships >= 20.0)
    if not bool(fleet_enemy.any()):
        z = torch.zeros(P, dtype=dtype, device=device)
        return torch.zeros(P, dtype=torch.bool, device=device), z

    fx = obs.f_x[fleet_enemy].to(dtype)
    fy = obs.f_y[fleet_enemy].to(dtype)
    angle = obs.f_angle[fleet_enemy].to(dtype)
    ships = obs.f_ships[fleet_enemy].to(dtype)
    ux = torch.cos(angle).view(-1, 1)
    uy = torch.sin(angle).view(-1, 1)
    vx = obs.x.view(1, P).to(dtype) - fx.view(-1, 1)
    vy = obs.y.view(1, P).to(dtype) - fy.view(-1, 1)
    proj = vx * ux + vy * uy
    perp = torch.abs(vx * uy - vy * ux)
    hit_band = obs.r.view(1, P).to(dtype) + 2.0
    plausible = (proj > 0.0) & (proj < 55.0) & (perp <= hit_band)
    plausible = plausible & obs.owned.view(1, P) & obs.alive.view(1, P)
    threat = torch.where(plausible, ships.view(-1, 1), torch.zeros_like(proj)).sum(dim=0)
    return threat > 0.0, threat + prod.to(dtype) * 2.0
...
def build_target_shortlist(
    ...
    if R > 0:
        ...
        incoming_mask, incoming_urgency = incoming_fleet_targets(obs)
        urgency = torch.maximum(urgency, torch.where(incoming_mask, incoming_urgency, torch.full_like(urgency, float("-inf"))))
        def_idx, def_exists = _candidate_indices(urgency, flip_mask | incoming_mask, R)
        def_exists = def_exists | incoming_mask[def_idx.clamp(0, P - 1)]
