...
_MULTISIZE_AGENT = _load_agent("m05_main.py", "h19_mapselect_m05")
...
def _use_multisize(obs, player: int) -> bool:
    ...
    for n in neutral:
        ...
        rows.append((value, en_eta - my_eta, my_eta))
    my_fast_values = [
        value for value, margin, my_eta in rows
        if margin >= 1.25 and my_eta <= 9.0
    ]
    best_fast_value = max(my_fast_values, default=0.0)
    my_fast_count = len(my_fast_values)
    return best_fast_value <= 54.947 and my_fast_count >= 2
