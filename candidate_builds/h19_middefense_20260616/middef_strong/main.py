...
CONFIG_2P_MIDDEF = dataclasses.replace(
    ProducerLiteConfig(),
    max_defensive_targets=8,
    max_regroup_time=9.0,
    regroup_pressure_delta_min=0.12,
    max_regroup_sources_per_lane=9,
    max_regroup_targets_per_source=11,
)


def _config_for_turn(player_count: int, step: int) -> ProducerLiteConfig:
    if int(player_count) >= 4:
        return CONFIG_4P
    if 45 <= int(step) <= 100:
        return CONFIG_2P_MIDDEF
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        config = _config_for_turn(mem.cached_player_count, step)
        row = run_turn(
