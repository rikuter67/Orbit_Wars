...
def build_target_shortlist(
    ...
    if R > 0:
        ...
        enemy_source_mask = obs.is_enemy & obs.alive
        owned_mask = obs.owned & obs.alive
        enemy_proximity = min_distance_to_targets(cache, enemy_source_mask, owned_mask, max_k=K_eta)
        pressure_urgency = torch.where(
            owned_mask,
            -enemy_proximity + prod.to(enemy_proximity.dtype) * 0.05,
            torch.full_like(enemy_proximity, float("-inf")),
        )
        urgency = torch.maximum(urgency, pressure_urgency)
        def_idx, def_exists = _candidate_indices(urgency, flip_mask | owned_mask, R)
