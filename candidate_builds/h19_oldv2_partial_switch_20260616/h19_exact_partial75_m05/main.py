...
def plan_lite_waves(
    *,
    ...
    multipliers = tuple(float(x) for x in config.size_multipliers)
    if multipliers != (1.0,):
        tier_parts = [
            _tier_candidates(
...
def _enemy_partial_launch_seen(obs, player_id: int, *, max_step: int, ratio_cut: float) -> bool:
    ...
    for fleet in _get(obs, "fleets", []) or []:
        ...
        if source is None or int(source[1]) != owner:
            continue
        if float(fleet[6]) / max(float(source[5]), 1.0) < float(ratio_cut):
            return True
    return False
...
class ProducerLiteMemory:
    def __init__(self) -> None:
        ...
        self.use_multisize: bool = False

    def reset(self) -> None:
        self.movement = None
        self.cached_player_count = None
        self.last_sparse_action_row = None
        self.use_multisize = False
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        if int(mem.cached_player_count) == 2 and bool(mem.use_multisize):
            config = dataclasses.replace(config, size_multipliers=(0.5, 1.0))
        row = run_turn(
...
def agent(obs):
    ...
    step = int(_get(obs, "step", 0) or 0)
    if step == 0:
        _RUNTIME.memory.use_multisize = False
    if not _RUNTIME.memory.use_multisize and _enemy_partial_launch_seen(
        obs, player_id, max_step=45, ratio_cut=0.75
    ):
        _RUNTIME.memory.use_multisize = True
