...
def plan_lite_waves(
    *,
    ...
    partial_risk_penalty: float = 2.0,
):
    ...
    if bool(partial_risk_active):
        score = score + cand_is_def.to(dtype) * float(partial_risk_penalty)
...
def run_turn(obs_tensors: dict, *, config: ProducerLiteConfig, player_count: int, memory) -> dict:
    ...
    entries = plan_lite_waves(
        ...
        partial_risk_penalty=getattr(memory, "partial_risk_penalty", 2.0),
    )
...
def _enemy_partial70_seen(obs, player_id: int, *, max_step: int = 45) -> bool:
...
class ProducerLiteMemory:
    def __init__(self) -> None:
        ...
        self.partial_risk_penalty: float = 2.0
