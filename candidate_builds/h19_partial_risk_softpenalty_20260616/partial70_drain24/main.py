...
def plan_lite_waves(
    *,
    ...
    player_count: int,
    partial_risk_active: bool = False,
    partial_risk_penalty: float = 2.4,
):
    ...
    if bool(partial_risk_active) and enemy_mass is not None:
        src_slots = cand_src.squeeze(-1).clamp(0, P - 1)
        src_pressure = enemy_mass[src_slots].to(dtype)
        src_ships_now = obs.ships[src_slots].to(dtype).clamp(min=1.0)
        drain_frac = (cand_send.squeeze(-1).to(dtype) / src_ships_now).clamp(min=0.0, max=1.0)
        pressure_frac = (src_pressure / (src_ships_now + 1.0)).clamp(min=0.0, max=1.0)
        score = score - float(partial_risk_penalty) * pressure_frac * drain_frac
...
def run_turn(obs_tensors: dict, *, config: ProducerLiteConfig, player_count: int, memory) -> dict:
    ...
    entries = plan_lite_waves(
        ...
        partial_risk_active=getattr(memory, "partial_risk_active", False),
        partial_risk_penalty=getattr(memory, "partial_risk_penalty", 2.4),
    )
...
def _enemy_partial70_seen(obs, player_id: int, *, max_step: int = 25) -> bool:
...
class ProducerLiteMemory:
    def __init__(self) -> None:
        ...
        self.partial_risk_active: bool = False
        self.partial_risk_penalty: float = 2.4

    def reset(self) -> None:
        self.movement = None
        self.cached_player_count = None
        self.last_sparse_action_row = None
        self.partial_risk_active = False
...
def agent(obs):
    ...
    step = int(_get(obs, "step", 0) or 0)
    if step == 0:
        _RUNTIME.memory.partial_risk_active = False
    if not _RUNTIME.memory.partial_risk_active and _enemy_partial70_seen(obs, player_id):
        _RUNTIME.memory.partial_risk_active = True
