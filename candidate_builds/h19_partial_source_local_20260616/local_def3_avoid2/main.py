...
def plan_lite_waves(
    *,
    ...
    player_count: int,
    partial_source_slots: set[int] | None = None,
    partial_local_radius: float = 25.0,
    partial_def_bonus: float = 3.0,
    partial_attack_penalty: float = 2.0,
):
    ...
    if partial_source_slots:
        local_targets = torch.zeros(P, dtype=torch.bool, device=device)
        d0 = cache.cross_dist[0].to(dtype)
        for slot in partial_source_slots:
            if 0 <= int(slot) < P:
                local_targets |= d0[int(slot)] <= float(partial_local_radius)
        local = local_targets[cand_tgt_slot.clamp(0, P - 1)]
        score = score + (local & cand_is_def).to(dtype) * float(partial_def_bonus)
        score = score - (local & ~cand_is_def).to(dtype) * float(partial_attack_penalty)
...
def run_turn(obs_tensors: dict, *, config: ProducerLiteConfig, player_count: int, memory) -> dict:
    ...
    entries = plan_lite_waves(
        ...
        partial_source_slots=getattr(memory, "partial_source_slots", None),
        partial_local_radius=getattr(memory, "partial_local_radius", 25.0),
        partial_def_bonus=getattr(memory, "partial_def_bonus", 3.0),
        partial_attack_penalty=getattr(memory, "partial_attack_penalty", 2.0),
    )
...
def _get(obs, key: str, default=None):
    return obs.get(key, default) if isinstance(obs, dict) else getattr(obs, key, default)


def _enemy_partial70_sources(obs, player_id: int, *, max_step: int = 45) -> set[int]:
    ...
    for fleet in _get(obs, "fleets", []) or []:
        ...
        if source is None or int(source[1]) != owner:
            continue
        ratio = float(fleet[6]) / max(float(source[5]), 1.0)
        if 0.65 <= ratio <= 0.75:
            out.add(source_id)
    return out
...
class ProducerLiteMemory:
    def __init__(self) -> None:
        ...
        self.partial_source_slots: set[int] = set()
        self.partial_local_radius: float = 25.0
        self.partial_def_bonus: float = 3.0
        self.partial_attack_penalty: float = 2.0

    def reset(self) -> None:
        self.movement = None
        self.cached_player_count = None
        self.last_sparse_action_row = None
        self.partial_source_slots = set()
...
def agent(obs):
    ...
    step = int(_get(obs, "step", 0) or 0)
    if step == 0:
        _RUNTIME.memory.partial_source_slots = set()
    _RUNTIME.memory.partial_source_slots.update(_enemy_partial70_sources(obs, player_id))
