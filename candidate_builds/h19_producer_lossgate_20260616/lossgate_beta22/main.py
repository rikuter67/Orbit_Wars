...
CONFIG_2P_LOSSGATE = dataclasses.replace(ProducerLiteConfig(), reinforce_size_beta=2.2)
...
def _use_lossgate(obs_tensors: dict, player_id: int) -> bool:
    best_fast_value, avg_abs_margin_top5 = _initial_map_features(obs_tensors, player_id)
    high_fast_risky = best_fast_value >= 90.0 and avg_abs_margin_top5 >= 20.0
    low_margin_risky = 50.0 <= best_fast_value <= 65.0 and avg_abs_margin_top5 <= 22.0
    mid_fast_risky = 70.0 <= best_fast_value <= 80.0 and avg_abs_margin_top5 >= 26.0
    return high_fast_risky or low_margin_risky or mid_fast_risky
...
class ProducerLiteMemory:
    def __init__(self) -> None:
        ...
        self.use_lossgate: bool = False
        self.last_sparse_action_row: dict | None = None

    def reset(self) -> None:
        self.movement = None
        self.cached_player_count = None
        self.use_lossgate = False
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        if mem.cached_player_count is None:
            ...
            if int(mem.cached_player_count) < 4:
                mem.use_lossgate = _use_lossgate(obs_tensors, int(obs_tensors["player"].item()))
        config = CONFIG_2P_LOSSGATE if int(mem.cached_player_count) < 4 and mem.use_lossgate else _config_for(mem.cached_player_count)
        row = run_turn(
