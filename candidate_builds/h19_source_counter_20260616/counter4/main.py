...
def plan_lite_waves(
    *,
    ...
    step_now = int(obs.step.flatten()[0].item())
    if int(player_count) == 2 and 35 <= step_now <= 90:
        ...
        if bool(enemy_f.any()):
            from_ids = fleets[..., 5]
            planet_ids = planets[..., 0]
            source_match = (planet_ids.view(-1, 1) == from_ids[enemy_f].view(1, -1)).any(dim=1)
            source_match = source_match & obs.is_enemy & obs.alive
            source_bonus = source_match[target_idx.clamp(0, P - 1)].to(dtype) * 4.0
            score = score + source_bonus[cand_tgt_short]
