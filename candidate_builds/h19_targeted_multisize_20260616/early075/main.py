...
def plan_lite_waves(
    *,
    ...
    step = int(obs_tensors["step"].reshape(-1)[0].item()) if "step" in obs_tensors else 0
    size_multipliers = config.size_multipliers if step <= 80 else (1.0,)
    tier_parts = [
        _tier_candidates(
            ...
            partial_target_mask=torch.ones(T, dtype=torch.bool, device=device),
